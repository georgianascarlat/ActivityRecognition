...
public class PersistUserInfo extends PApplet {
	...
	private String rootDirectory = "/home/gia/An4/Licenta/kinect/data/recorded/points/";
	
	private Floor floor;
	private floor.Floor the_floor = null;
	int heigthOffset = 0, widthOffset = 0;
	private int widthParts = 8;
	...
	private void showFloorPoints(List<Point3f> planPoints, PVector proj) {
		...
		for(i=0;i<planPoints.size();i++){
			pp = new PVector(planPoints.get(i).x,planPoints.get(i).y,planPoints.get(i).z);
	...
	private void writeJointPositionsToFile(int userId) {
		...
		try {
			...
			writeJoint(userId, out, SimpleOpenNI.SKEL_LEFT_SHOULDER,"LEFT_SHOULDER");
			writeJoint(userId, out, SimpleOpenNI.SKEL_LEFT_ELBOW,"LEFT_ELBOW");
			writeJoint(userId, out, SimpleOpenNI.SKEL_LEFT_HAND,"LEFT_HAND");
			
			writeJoint(userId, out, SimpleOpenNI.SKEL_RIGHT_SHOULDER,"RIGHT_SHOULDER");
			writeJoint(userId, out, SimpleOpenNI.SKEL_RIGHT_ELBOW,"RIGHT_ELBOW");
	...
	private void showFloorHighlight(String fileName2) throws IOException {
		...
		Snapshot snapshot ;
		if(the_floor != null)
			snapshot = new Snapshot(fileName2,widthParts,heigthParts,the_floor);
		else{
		...
		for (int line = 0; line < heigthParts; line++) {
            for (int column = 0; column < widthParts; column++) {
                ...
                rect(x+widthOffset, y+heigthOffset, map.getWidthChunk(), map.getHeightChunk());
	...
	private void writeJoint(int userId, BufferedWriter out, int joint, String name)
			throws IOException {
		...
		out.append(name+" "+jointPos.x+ " "+ jointPos.y+ " " + jointPos.z +"\n");
	}
